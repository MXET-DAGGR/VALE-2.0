...
from jelly_geo import NoGo, Destinations
...
class Bearing(object):
    ...
    def __init__(self, dest):
        ...
        #gpsData=gps.readPosition()
        #self.currentLoc = Point(gpsData[0],gpsData[1])
        #self.currentLoc = Point(30.617073,-96.341687)  # entrance to the PIC
        gpsData=gps.readPosition()
        #while( gpsData == None | gpsData==[0,0]):
         #   print("drop invalid data")
          #  time.sleep(5)
        self.currentLoc = Point(gpsData[0],gpsData[1])
        self.arrived = False
    ...
    def checkWaypt(self):
        ...
        if x1<=self.currentLoc.x<=x2 and y1<=self.currentLoc.y<=y2:
            ...
            if self.currentWaypt_iterator >= len(self.waypts):
                ...
                print('Waypts length: ',len(self.waypts))
                print('Current iterator: ', self.currentWaypt_iterator)
                self.currentWaypt = self.waypts[self.currentWaypt_iterator]
